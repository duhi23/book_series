...
\begin{definicion}
$\left( {X_{t} ,t\in \Z} \right)$ centrado, de segundo orden, se dice un \emph{proceso 
lineal}\index{Proceso lineal!Definici\'{o}n} si se puede expresar como:
\begin{equation}
\label{eq1}
...
\begin{observacion}
\begin{enumerate}
\item (\ref{eq1}) no es necesariamente la descomposici\'{o}n de Wold de $\left( {X_{t} } \right)_{t\in \Z} $.
\item $u_{t}\bot u_{t-1}$ pues:
\[
...
\item $(X_{t} )$ d.e., pues es centrado y adem\'{a}s:
\begin{align*}
...
\item Se pasa de $\left( {u_{t} } \right)$ a $\left( {X_{t} } \right)$ por una transformaci\'{o}n lineal\index{Proceso lineal!Filtro lineal}, llamada \emph{filtro lineal}.
\item Sup\'{o}ngase que $\sum_j {\left| {\psi_{j} } \right|<\infty } $
\[
...
\]
entonces existe una medida espectral $\mu $ de $\left( {X_{t} } \right)$, 
absolutamente continua, con una densidad $f$ continua 
(si $f\geq 0$).
\end{enumerate}
...
\begin{enumerate}
 \item $u_{t} \in \mu_{t} $ 
 \item \begin{equation}
        X_{t} =\sum_{j=1}^\infty {\pi_{j} X_{t-j} } +u_{t} =\widehat{X}_{t} +u_{t} \qquad t\in \Z,\text{ con }\pi_{j} =-\pi_{j}'     
       \end{equation}
	...
	\[
	 X_{t} =\sum_{j=1}^p \pi_{j} X_{t-j} +u_{t} ,\qquad t\in \Z
	\]
...
\paragraph{Notaciones:} Consid\'{e}rese el operador\index{Operador!Retardo} de 
retardo $B$ (Backward es su expresi\'{o}n en ingl\'{e}s), definido por $BX_{t} 
=X_{t-1} $ y sean:
\[
...
\[
\E\left( {Z_{s} Z_{t} } \right)=\sum_{j,j'} {c_{j} 
\bar{{c}}_{j'} \E\left( {Y_{s-j} Y_{t-j'} } \right)} 
=\sum_{j,j'} {c_{j} \bar{{c}}_{j'} } \int\limits_{\left[ {-\pi 
,\pi } \right]} {\cos \lambda \left( {\left( {t-s} \right)+\left( {j'-j} 
\right)} \right)d\mu \left( \lambda \right)} 
\]
...
\]
\[
\E\left( {Z_{s} Z_{t} } \right)=\int {\Re\left[ {\sum_{j,j'} {c_{j} 
c_{j'} e^{i\lambda \left( {\left( {t-s} \right)+\left( {j'-j} \right)} 
\right)}} } \right]\,d\mu \left( \lambda \right)} 
=\int {\Re e^{i\lambda \left( {t-s} \right)}\left| {\sum_j {c_{j} 
e^{i\lambda j}} } \right|^{2}d\mu \left( 
\lambda \right)} 
\]
\[
...
\]
El resultado se deduce por la unicidad de la representaci\'{o}n espectral de 
los $\gamma_{t} $.
\end{proof}

\paragraph{Aplicaci\'{o}n: } Sea $X_{t} =\sum_{j=0}^\infty {\psi_{j } 
u_{t-j} } $ proceso lineal, con 
\[
\sum_{j=0}^\infty \left| \psi_{j} \right| <\infty 
\]
Puesto que $\left( {u_{t} } \right)$ ruido blanco, la medida espectral es 
$\frac{\sigma^{2}}{2\pi }d\lambda ,$ entonces la medida espectral de $X_{t} 
$ se escribe:
\[
f_{X} \left( \lambda \right)=
\frac{\sigma^{2}}{2\pi }\left| {\sum_j {\psi_{j} e^{i\lambda j}} } \right|^{2}d\lambda 
\]
...
\section{Funciones de Autorrelaci\'{o}n Simple y Parcial de un P.E.S.O.}
...
(0)}
\]
Puesto que la varianza de $X_{t}$ es constante, este es el coeficiente de 
correlaci\'{o}n lineal entre $X_{t} y X_{t+l}$.
\end{definicion}

\begin{observacion}
 Para realizar estimaciones, se supondr\'{a} 
que se dispone de una observaci\'{o}n de cada una de las variables 
$X_{1},\ldots , X_{T}$ 

Dado que las covarianzas se conservan por saltos, $\rho \left( l \right)$ 
puede estimarse por:
\[
\widehat{\rho }(l)=\frac{\displaystyle \sum_{t=l+1}^T 
(X_{t}-\bar{X})(X_{t-l}-\bar{X}) }{\displaystyle \sum_{t=1}^T 
(X_{t}-\bar{X})^{2} }
\]
\end{observacion}

\begin{definicion}
 Se llama funci\'{o}n\index{Funci\'{o}n!De autocorrelaci\'{o}n parcial} de autocorrelaci\'{o}n parcial de orden $l$ (denotada por $r\left( l \right)o r_{l}$) de 
...
$p$\index{Procesos AR} (notaci\'{o}n: $AR(p)$ \index{Procesos AR!Orden $p$}).
\end{definicion}
...
llama la innovaci\'{o}n\index{Innovaci\'{o}n} en el instante $t$.
...
autorrelaci\'{o}n\index{Procesos AR!Coeficiente de autocorrelaci\'{o}n} es:
\[
\rho \left( l \right)=\frac{\gamma (l)}{\gamma 
...
el coeficiente de autocorrelaci\'{o}n parcial\index{Procesos AR!Coeficiente 
de autocorrelaci\'{o}n parcial}, entonces:
\[
\left\{ {\begin{array}{l}
 r_{p} =\pi_{p} \ne 0 \\ 
